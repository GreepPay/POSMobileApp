    ...
    >
      ...
      >
        ...
        <div v-else class="w-full flex flex-col px-4">
          <template
            v-for="(delivery, index) in currentDeliveries"
          ...
          <!-- Empty State -->
          <div
            v-if="currentDeliveries.length === 0"
...
export default defineComponent({
  ...
  setup() {
    ...
    const currentDeliveries = computed(() => {
      ...
      if (activeTab.value === "active") {
        ...
        const activeDeliveries = sortedDeliveries.filter((delivery: any) => {
          ...
          return !["delivered", "completed", "cancelled", "failed"].includes(
            status
          );
        });
    ...
    watch(
      () => Logic.Commerce.ManyBusinessDeliveries,
      () => {
        updateTrigger.value++; // Force reactive update
      },
      { deep: true, immediate: true }
