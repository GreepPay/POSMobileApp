    ...
    <subpage-layout title="Request Money">
      ...
      >
        <div class="w-full flex flex-col">
          <app-button
            variant="secondary"
            :class="`!py-4 ${paymentData?.amount ? 'opacity-100' : 'opacity-50'}`"
  ...
  setup() {
    ...
    const continueToNext = () => {
      if ((paymentData.value?.amount || 0) > 0) {
        Logic.Common.GoToRoute(`/request/qr?amount=${paymentData.value?.amount}&currency=${paymentData?.value?.currency}&narration=${paymentData.value?.narration || ""}`);
      }
    };
