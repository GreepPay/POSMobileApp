  ...
  setup() {
    ...
    const loadOrders = async () => {
      try {
        ...
        console.log("P2P orders loaded:", Logic.Wallet.ManyP2pOrders?.data?.length);
    ...
    const goToOrder = (uuid: string) => {
      if (uuid) {
        Logic.Common.GoToRoute(`/p2p-orders/${uuid}`);
      }
    };
    ...
    const currentOrders = computed(() => {
      ...
      if (activeTab.value === "active") {
        ...
        console.log("Active P2P orders:", activeOrders.length);
        return activeOrders;
      } else {
        ...
        console.log("History P2P orders:", historyOrders.length);
