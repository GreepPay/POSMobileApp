              ...
              <div class="w-full flex flex-row items-center">
                ...
                <div
                  ...
                  :style="`background-color: ${colorByStatus(
                    order.status.toLowerCase() as 'success' | 'failed' | 'pending'
                  )} !important;`"
                ></div>
                <app-normal-text
...
import { ExchangeOrder, User } from "@greep/logic/src/gql/graphql";
import { computed } from "vue";

export default defineComponent({
  name: "P2pOrderIndexPage",
  components: {
    DefaultPageLayout,
    AppTabs,
    AppIcon,
  ...
  setup() {
    ...
    const ManyP2pOrders = ref(Logic.Wallet.ManyP2pOrders);
    ...
    const loadOrders = async () => {
      try {
        ...
        // Load P2P orders
        console.log("Loading P2P orders...");

        await Logic.Wallet.GetP2pOrders(1, 50);

        console.log(
          "P2P orders loaded:",
          Logic.Wallet.ManyP2pOrders?.data?.length
        );
        ...
        error.value = "Failed to load P2P orders. Please try again.";
        console.error("Error loading P2P orders:", err);
      } finally {
    ...
    const goToOrder = (uuid: string) => {
      if (uuid) {
        Logic.Common.GoToRoute(`/exchange-orders/${uuid}`);
      }
    };
    ...
    // Computed for current P2P orders with immediate reactivity
    const setCurrentOrder = () => {
      ...
      finalOrders.forEach((order: ExchangeOrder) => {
        const details = {
          uuid: order.uuid,
          title: formatOrderTitle(order),
          label: Logic.Common.fomartDate(order.created_at, "MMM DD, hh:mm A"),
    ...
    onMounted(() => {
      Logic.Wallet.watchProperty("ManyP2pOrders", ManyP2pOrders);
      setCurrentOrder();
    });
    ...
    // Watch for changes in P2P orders data to trigger re-computation
    watch(
      ManyP2pOrders,
      () => {
