        ...
        <subpage-layout title="Business Locations">
            <div class="w-full flex flex-col justify-start h-full pt-1">
                <div class="w-full flex flex-col items-center justify-start px-4 h-full pt-4">
                    ...
                    <template v-else>
                        <template v-for="(address, index) in deliveryAddresses" :key="index">
                            <div
                                ...
                                <div class="flex flex-row justify-end">
                                    <div @click="goToEditAddress(address.uuid)" class="cursor-pointer">
...
export default defineComponent({
    ...
    setup() {
        ...
        const deliveryAddresses = reactive<Array<{
            uuid: string;
            name: string;
            description?: string;
        ...
        const paginatorInfo = ref<any>(Logic.User.DeliveryAddresses?.paginatorInfo || {});

        const refreshData = async () => {
            try {
                await Logic.User.GetDeliveryAddresses(20, 1);
                deliveryAddresses.splice(0, deliveryAddresses.length, ...(Logic.User.DeliveryAddresses?.data || []));
