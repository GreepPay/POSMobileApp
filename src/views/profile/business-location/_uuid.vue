    ...
    <app-wrapper>
        <subpage-layout title="Edit Location">
            <div class="w-full flex flex-col justify-start px-4 h-full pt-4">
                <div class="w-full flex flex-col space-y-4" v-if="!hideForm">
                    ...
                    <div class="w-full flex flex-col">
                        <app-text-field :has-title="true" title="Address Name" type="text"
                            placeholder="Enter address name (e.g., HQ, Branch Office)" v-model="formData.name"
                            :error-message="validationErrors.name" @input="validateField('name', formData.name)" />
                    </div>

                    <!-- Delivery Location Area -->
                    <div class="w-full flex flex-col" v-if="showDeliverySelector">
                        <app-select :placeholder="'Select Delivery Location Area'" :hasTitle="false"
                            :paddings="'py-4 !px-4'" :options="deliveryLocationOptions" ref="deliveryArea"
                            usePermanentFloatingLabel v-on:update:model-value="
                                (data) => {
                                    formData.delivery_location_id = data;
                                }
                            " v-model="deliveryLocationId"
                            @update:model-value="validateField('delivery_location_id', formData.delivery_location_id || '')"
                            auto-complete>
                        </app-select>
                    </div>
                    ...
                    <!-- Google Maps Link  -->
                    <div class="w-full flex flex-col">
                        <app-text-field :has-title="true" title="Google Maps Link" type="url"
                            placeholder="Google Map Link" v-model="formData.google_map_link" />
                    </div>

                    <!-- Description -->
                    <div class="w-full flex flex-col">
                        <app-text-field :has-title="true" title="Description" type="text" placeholder="Description"
                            v-model="formData.description" :is-textarea="true" text-area-row="4" />
                    </div>
            ...
            <div class="w-full fixed bg-white dark:bg-black bottom-0 left-0 pt-4 px-4" :style="`${getBottomPadding}`">
                <div class="w-full flex flex-col">
                    <app-button :class="`!py-4`" :loading="loadingState" @click="updateDeliveryAddress"
...
export default defineComponent({
    ...
    setup() {
        ...
        const isFormValid = () => {
            ...
            const areaValid = validateField("delivery_location_id", formData.delivery_location_id || "");
        ...
        const fetchDeliveryLocations = async () => {
            try {
                ...
                if (Logic.Commerce.ManyDeliveryLocations?.data && Logic.Commerce.ManyDeliveryLocations.data.length > 0) {
                ...
                await Logic.Commerce.GetDeliveryLocations(1, 100, "CREATED_AT", "DESC", "");
                const locations = Logic.Commerce.ManyDeliveryLocations?.data || [];
