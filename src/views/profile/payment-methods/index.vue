            ...
            <div class="w-full flex flex-col justify-start h-full pt-1">
                <div class="w-full flex flex-col items-center justify-start px-4 h-full pt-4">
                    ...
                    <div class="w-full flex flex-col pb-4">
                        <div @click="goToAddPaymentMethod"
                        ...
                        <template v-for="(method, index) in paymentMethods" :key="index">
                            <div
                                ...
                                <div class="flex flex-row space-x-3">
                                    <div @click="goToEditPaymentMethod(method.uuid)" class="cursor-pointer">
                                        <app-icon name="edit" custom-class="h-[20px] text-gray-400" />
                                    </div>
                                </div>
                            </div>
                        </template>
...
export default defineComponent({
    ...
    setup() {
        ...
        const paymentMethods = reactive<any[]>(Logic.Wallet.ManyP2pPaymentMethods?.data || []);

        const paginatorInfo = ref<any>(Logic.Wallet.ManyP2pPaymentMethods?.paginatorInfo || {});

        const refreshData = async () => {
            try {
                await Logic.Wallet.GetMyP2pPaymentMethods(20, 1);
                paymentMethods.splice(0, paymentMethods.length, ...(Logic.Wallet.ManyP2pPaymentMethods?.data || []));
                paginatorInfo.value = Logic.Wallet.ManyP2pPaymentMethods?.paginatorInfo || {};
            } catch (error) {
        ...
        const goToEditPaymentMethod = (uuid: string) => {
            if (uuid) {
                console.log("✏️ Editing payment method with UUID:", uuid);
                Logic.Common.GoToRoute(`/profile/payment-methods/edit/${uuid}`);
            } else {
                console.warn("⚠️ No UUID provided for editing payment method");
            }
        };
