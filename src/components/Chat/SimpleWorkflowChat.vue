<template>
    <app-wrapper>
        <div class="w-full flex flex-col lg:text-sm mdlg:text-[12px] relative text-xs font-poppins overflow-y-auto h-full"
            ...
            <div v-if="showProofUpload && !hasUploadedProof"
                class="fixed bottom-20 left-1/2 transform -translate-x-1/2 z-40 animate-bounce">
                <app-button @click="showProofModal = true" class="!bg-primary !text-white !px-6 !py-3 !rounded-full !shadow-lg !border-2 !border-white
                                  flex items-center space-x-2 hover:!bg-primary-dark transition-all duration-300">
            ...
            <div class="fixed bottom-0 left-0 right-0 bg-white z-30">
                ...
                <chat-bottom-bar :conversation="conversationDataWithStage" :send-message="sendMessage"
                    :last-a-i-message="lastAIMessage" :is-processing="isProcessing" :authUser="Logic.Auth.AuthUser"
                    :disabled="isProcessing" :show-address-mode="false" :order-confirmed="true"
                    :proof-uploaded="proofUploaded" @click-address-input="() => { }"
            ...
            <!-- Bank Transfer Modal -->
            <bank-transfer-modal v-if="activeModal === 'bank_transfer'" :show="true"
                :savedBankAccounts="savedBankAccounts" :isProcessing="isProcessing"
            ...
            <!-- Proof Upload Modal -->
            <proof-upload-modal :show="showProofModal" @cancel="handleProofCancel"
                @upload-success="handleProofUploadSuccess" @upload-error="handleProofUploadError" />
    ...
    setup(props) {
        ...
        const setupChatWebSocket = () => {
            ...
            setupWebSocketListeners(conversationUuid, {
                ...
                onMessageCreated: (data) => {
                    console.log("📨 Simple WorkflowChat received WebSocket message:", data);
        ...
        const handleProofUploadSuccess = async (uploadData: { fileUrl: string; fileName: string; fileType: string }) => {
            ...
            try {
                ...
                Logic.Common.showAlert({
                    show: true,
                    message: "Proof uploaded but failed to notify user. Please contact support.",
                    type: "error"
                });
            }
            ...
            await handleProofUploadComplete([{ name: uploadData.fileName, type: uploadData.fileType }] as File[]);
        ...
        // Cleanup
        onUnmounted(() => {
            console.log("👋 Simple WorkflowChat unmounting - cleaning up WebSocket...");
            cleanupWebSocket();
        });
